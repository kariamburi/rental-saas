...
import { CalendarDays, Edit, Gauge, User, Wallet, X } from "lucide-react";
...
export default function EditMeterReadingModal({
    ...
    const [tenantId, setTenantId] = useState(reading.tenantId);
    const [type, setType] = useState(reading.type);
    const [previousReading, setPreviousReading] = useState(String(reading.previousReading));
    const [currentReading, setCurrentReading] = useState(String(reading.currentReading));
    const [ratePerUnit, setRatePerUnit] = useState(String(reading.ratePerUnit));
    const [billingMonth, setBillingMonth] = useState(reading.billingMonth);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            if (!res.ok || !data.ok) {
                setError(data.error || "Failed to update meter reading");
                return;
            }
                    ...
                    <div className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-[2rem] bg-white shadow-2xl">
                        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
                            <div>
                                ...
                                <p className="text-sm text-slate-500">
                                    Update water or electricity usage
                                </p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex-1 space-y-5 overflow-y-auto p-6">
                            ...
                            <div className="grid gap-5 md:grid-cols-2">
                                ...
                                <Input icon={CalendarDays} label="Billing Month" value={billingMonth} onChange={setBillingMonth} type="month" />
                                <Input icon={Gauge} label="Previous Reading" value={previousReading} onChange={setPreviousReading} type="number" />
                                <Input icon={Gauge} label="Current Reading" value={currentReading} onChange={setCurrentReading} type="number" />
                                <Input icon={Wallet} label="Rate Per Unit" value={ratePerUnit} onChange={setRatePerUnit} type="number" />
                            </div>
