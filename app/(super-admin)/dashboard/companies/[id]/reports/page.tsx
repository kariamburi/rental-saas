...
import { DoorOpen, FileText, Home, TrendingUp, Users, Wallet } from "lucide-react";
...
export default async function CompanyReportsPage({
    ...
    const [units, tenants, invoices, payments, properties, expenses] =
        await Promise.all([
            prisma.unit.findMany({ where: { companyId: id } }),
            prisma.tenant.findMany({ where: { companyId: id } }),
            prisma.invoice.findMany({ where: { companyId: id } }),
            prisma.payment.findMany({ where: { companyId: id } }),
            prisma.property.findMany({
                where: { companyId: id },
                include: { units: true },
            }),
            prisma.expense.findMany({ where: { companyId: id } }),
        ]);
                            ...
                            {properties.map((property) => (
                                <tr key={property.id} className="transition hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4 font-semibold text-amber-700">
                                        {property.units.filter((u) => u.status === "MAINTENANCE").length}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
